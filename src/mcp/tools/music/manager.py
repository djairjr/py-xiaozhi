...
Responsible for the initialization, configuration and MCP tool registration of music tools"""
...
class MusicToolsManager:
    ...
    def __init__(self):
        ...
        logger.info("[MusicManager] Music tool manager initialization")

    def init_tools(self, add_tool, PropertyList, Property, PropertyType):
        """Initialize and register all music tools."""
        try:
            logger.info("[MusicManager] Start registering music tools")

            # Get the music player singleton instance
            self._music_player = get_music_player_instance()

            # Sign up for the search and play tool
    ...
    def _register_search_and_play_tool(
        ...
        async def search_and_play_wrapper(args: Dict[str, Any]) -> str:
            ...
            return result.get("message", "Search playback completed")
        ...
        logger.debug("[MusicManager] Registered search and playback tool successfully")
    ...
    def _register_play_pause_tool(self, add_tool, PropertyList):
        ...
        async def play_pause_wrapper(args: Dict[str, Any]) -> str:
            ...
            return result.get("message", "Playback status switching completed")
        ...
        logger.debug("[MusicManager] Registered playback and pause tool successfully")
    ...
    def _register_stop_tool(self, add_tool, PropertyList):
        ...
        logger.debug("[MusicManager] Registration and stop tool successful")
    ...
    def _register_seek_tool(self, add_tool, PropertyList, Property, PropertyType):
        ...
        async def seek_wrapper(args: Dict[str, Any]) -> str:
            ...
            return result.get("message", "Jump completed")
        ...
        logger.debug("[MusicManager] Registered jump tool successfully")

    def _register_get_lyrics_tool(self, add_tool, PropertyList):
        """Sign up to get the lyrics tool."""
        ...
        logger.debug("[MusicManager] Successfully registered to obtain lyrics tool")

    def _register_get_status_tool(self, add_tool, PropertyList):
        """Sign up for the status tool."""
        ...
        async def get_status_wrapper(args: Dict[str, Any]) -> str:
            ...
            if result.get("status") == "success":
                ...
                status_info.append(
                    f"Playing status: {'Playing' if result.get('is_playing') else 'Stopped'}"
                )
                if result.get("is_playing"):
                    if result.get("paused"):
                        status_info.append("Status: Suspended")
                    else:
                        status_info.append("Status: Playing")

                    duration = result.get("duration", 0)
                    position = result.get("position", 0)
                    progress = result.get("progress", 0)

                    status_info.append(f"Playing duration: {self._format_time(duration)}")
        ...
        logger.debug("[MusicManager] Registered status tool successfully")

    def _register_get_local_playlist_tool(
        self, add_tool, PropertyList, Property, PropertyType
    ):
        """Sign up to get the local playlist tool."""
        ...
        async def get_local_playlist_wrapper(args: Dict[str, Any]) -> str:
            ...
            if result.get("status") == "success":
                ...
                if playlist:
                    playlist_text = f"Local music playlist ({total_count} songs in total):\n"
                    playlist_text += "\n".join(playlist)
                    return playlist_text
                else:
                    return "No music files in local cache"
            else:
                return result.get("message", "Failed to obtain local playlist")
        ...
        add_tool(
            (
                ...
                "'Chrysanthemum Terrace - Jay Chou', call search_and_play with song_name='Chrysanthemum Terrace'.",
                refresh_props,
                get_local_playlist_wrapper,
            )
        )
        logger.debug("[MusicManager] Successfully registered to obtain local playlist tool")
...
def get_music_tools_manager() -> MusicToolsManager:
    ...
    if _music_tools_manager is None:
        ...
        logger.debug("[MusicManager] Create a music tool manager instance")
