...
Provide specific system tool function implementation"""
...
async def get_system_status(args: Dict[str, Any]) -> str:
    ...
    try:
        logger.info("[SystemTools] Start getting system status")

        # Use a thread pool to perform synchronous device status acquisition to avoid blocking the event loop
        ...
        logger.info("[SystemTools] System status obtained successfully")
        return json.dumps(status, ensure_ascii=False, indent=2)

    except Exception as e:
        logger.error(f"[SystemTools] Failed to obtain system status: {e}", exc_info=True)
...
async def set_volume(args: Dict[str, Any]) -> bool:
    ...
    try:
        volume = args["volume"]
        logger.info(f"[SystemTools] Set volume to {volume}")
        ...
        # Set the volume directly using VolumeController
        ...
        if not VolumeController.check_dependencies():
            logger.warning("[SystemTools] The volume control dependency is incomplete and the volume cannot be set.")
            return False
        ...
        logger.error("[SystemTools] volume parameter missing")
        return False
    except Exception as e:
...
async def _get_audio_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if VolumeController.check_dependencies():
            volume_controller = VolumeController()
            # Use thread pool to obtain volume to avoid blocking
...
def _get_application_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # The value of DeviceState is directly a string, and there is no need to access the .name attribute.
        ...
        logger.warning(f"[SystemTools] Failed to obtain application status: {e}")
